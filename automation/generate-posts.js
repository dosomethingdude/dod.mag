...
- 실존 인물(배우, 가수, 운동선수, 기업인 등)에 대해 실제로 존재하지 않는 작품(드라마, 영화, 앨범, 노래 등)을 지어내지 마세요
- 실존 인물이 실제로 하지 않은 발언이나 인터뷰를 꾸며내지 마세요
- 실존 인물이 받지 않은 수상 내역, 기록, 성과를 날조하지 마세요
- 실존 인물의 사생활(연애, 결혼, 가족 등)을 추측하거나 창작하지 마세요
- 인물에 관한 글을 쓸 때 확인된 대표작/실제 활동만 언급하세요. 불확실하면 구체적 작품명 대신 "여러 작품에서"처럼 표현하세요
- 위 조건을 충족할 충분한 검증 가능한 정보가 없으면 반드시 "skip": true로 응답하세요
